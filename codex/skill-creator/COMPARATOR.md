---
name: "codex/skill-creator comparator"
description: "Use when the model needs to compare version directories inside codex/skill-creator. This document teaches only pairwise ordering for two concrete versions."
---

# codex/skill-creator Comparator

## Purpose

Define how to compare two published versions under `codex/skill-creator/`.
...
Each version directory under `codex/skill-creator/` is an exact published snapshot.
...
- If a version directory name does not match the expected `v`-prefixed SemVer form, stop using this comparator and investigate the repo's actual versioning rule.
- Treat version directories as exact artifacts already published in the registry.

## Rationale

`codex/skill-creator` uses `v`-prefixed SemVer 2.0.0 for registry releases, so ordering comes from SemVer 2.0.0 precedence after removing the leading `v`.
