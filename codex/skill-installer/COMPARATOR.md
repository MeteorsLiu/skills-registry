---
name: "codex/skill-installer comparator"
description: "Use when the model needs to compare version directories inside codex/skill-installer. This document teaches only pairwise ordering for two concrete versions."
---

# codex/skill-installer Comparator

## Purpose

Define how to compare two published versions under `codex/skill-installer/`.
...
Each version directory under `codex/skill-installer/` is an exact published snapshot.
...
`codex/skill-installer` follows `v`-prefixed SemVer 2.0.0, so ordering comes from SemVer 2.0.0 precedence after removing the leading `v`.
