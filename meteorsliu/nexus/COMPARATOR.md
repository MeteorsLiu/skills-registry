---
name: "meteorsliu/nexus comparator"
description: "Use when the model needs to compare version directories inside meteorsliu/nexus. This document teaches only pairwise ordering for two concrete versions."
---

# meteorsliu/nexus Comparator

## Purpose

Define how to compare two published versions under `meteorsliu/nexus/`.
...
Each version directory under `meteorsliu/nexus/` is an exact published snapshot.
...
`meteorsliu/nexus` uses `v`-prefixed SemVer 2.0.0 for registry releases, so ordering comes from SemVer 2.0.0 precedence after removing the leading `v`.
